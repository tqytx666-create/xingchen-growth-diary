...
const phase = ref('flash')   // flash → reveal
const bursts = ref([])
onMounted(() => {
  // 星爆粒子
  for (let i = 0; i < 14; i++) {
  ...
  setTimeout(() => { phase.value = 'reveal' }, 450)
})
</script>
<template>
  <div class="modal" @click.self="emit('close')">
    <div class="evo-flash" :class="{ gone: phase !== 'flash' }"></div>
    <div class="modal-card" :class="phase">
...
.modal-card.reveal { animation: evoPop .6s cubic-bezier(.2,1.5,.4,1); }
@keyframes evoPop { 0%{transform:scale(.4);opacity:0;} 60%{transform:scale(1.08);} 100%{transform:scale(1);opacity:1;} }
.evo-dog-wrap { position: relative; width: 180px; height: 180px; margin: 0 auto 14px; display: grid; place-items: center; }
.evo-dog { width: 100%; height: 100%; object-fit: contain; filter: drop-shadow(0 0 22px rgba(255,216,107,.75)); animation: evoReveal .7s ease; }
@keyframes evoReveal { 0%{transform:scale(.3) rotate(-12deg);opacity:0;} 100%{transform:scale(1) rotate(0);opacity:1;} }
.evo-ring {
